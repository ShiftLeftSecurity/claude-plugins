    ...
    var ClientRequest = class extends Writable {
      constructor(input2, options, callback) {
        ...
        if (typeof input2 === "string") {
          input2 = urlToHttpOptions(new URL3(input2));
        } else if (input2 instanceof URL3) {
...
var require_auto = __commonJS({
  "../packages/curness-core/node_modules/http2-wrapper/source/auto.js"(exports2, module2) {
    ...
    module2.exports = async (input2, options, callback) => {
      if (typeof input2 === "string") {
        input2 = urlToHttpOptions(new URL3(input2));
      } else if (input2 instanceof URL3) {
        input2 = urlToHttpOptions(input2);
...
var init_options = __esm({
  "../packages/curness-core/node_modules/got/dist/source/core/options.js"() {
    ...
    Options = class _Options {
      ...
      _merging = false;
      _init;
      constructor(input2, options, defaults2) {
        assertAny2("input", [distribution_default.string, distribution_default.urlInstance, distribution_default.object, distribution_default.undefined], input2);
        ...
        if (input2 instanceof _Options || options instanceof _Options) {
        ...
        try {
          if (distribution_default.plainObject(input2)) {
            try {
              this.merge(input2);
              this.merge(options);
            } finally {
...
// ../packages/curness-core/lib/ocularReady.js
var ocularReady_exports = {};
__export(ocularReady_exports, {
  ensureOcularReady: () => ensureOcularReady
});
async function ensureOcularReady(options = {}) {
  const dataDir = options.dataDir;
  const promptMessage = options.promptMessage;
  const onLog = options.onLog || (() => {
  });
  if (!dataDir || typeof promptMessage !== "string") {
    return false;
  }
  if (isOcularInstalled(dataDir)) {
    return true;
  }
  const answer = await import_vscode.default.window.showInformationMessage(
    promptMessage,
    "Download",
    "Cancel"
  );
  ...
  return await import_vscode.default.window.withProgress(
    {
      ...
      title: "Downloading Harness SAST and SCA dependencies",
      cancellable: true
    },
    async (progress, token) => {
      ...
      try {
        const ok = await ensureOcularInstalled(dataDir, {
          analyzeScriptSource,
          onProgress: (msg) => {
            onLog(msg);
        ...
        if (ok) {
          onLog("Dependencies downloaded and extracted successfully");
        }
        return ok;
      } catch (error) {
        onLog(`ERROR: Failed to download dependencies: ${error.message}`);
        if (token && token.isCancellationRequested) {
          import_vscode.default.window.showWarningMessage("Download cancelled");
        } else {
          import_vscode.default.window.showErrorMessage(`Failed to download dependencies: ${error.message}`);
        }
        return false;
      }
    }
...
var import_path5, import_vscode, import_url2, __dirname2, analyzeScriptSource;
var init_ocularReady = __esm({
  "../packages/curness-core/lib/ocularReady.js"() {
    import_path5 = __toESM(require("path"), 1);
    import_vscode = __toESM(require("vscode"), 1);
    import_url2 = require("url");
    init_ocular();
    __dirname2 = import_path5.default.dirname((0, import_url2.fileURLToPath)(__import_meta_url__));
    analyzeScriptSource = import_path5.default.join(__dirname2, "..", "analyze.sc");
  }
});

// src/claude-code-hook-handler.js
var import_path7 = __toESM(require("path"), 1);

// ../packages/curness-core/index.js
var curness_core_exports = {};
__export(curness_core_exports, {
  CURNESS_EXTENSION_ID: () => CURNESS_EXTENSION_ID,
  OCULAR_VERSION: () => OCULAR_VERSION2,
  SUPPORTED_EXTENSIONS: () => SUPPORTED_EXTENSIONS,
  SUPPORTED_LANGUAGE_IDS: () => SUPPORTED_LANGUAGE_IDS,
  appendPendingFindings: () => appendPendingFindings2,
  buildFollowupMessage: () => buildFollowupMessage2,
  checkOcularUpdate: () => checkOcularUpdate2,
  createHookSocketServer: () => createHookSocketServer2,
  ensureAnalyzeScript: () => ensureAnalyzeScript2,
  ensureHookAuthToken: () => ensureHookAuthToken2,
  ensureOcularInstalled: () => ensureOcularInstalled2,
  ensureOcularReady: () => ensureOcularReady2,
  extractPathsFromEditWriteToolInput: () => extractPathsFromEditWriteToolInput,
  findingsToDiagnostics: () => findingsToDiagnostics,
  formatFindingsSummary: () => formatFindingsSummary,
  getClaudePendingPath: () => getClaudePendingPath2,
  getCursorHookHandlerPath: () => getCursorHookHandlerPath2,
  getCursorStopHandlerPath: () => getCursorStopHandlerPath2,
  getInstalledOcularVersion: () => getInstalledOcularVersion2,
  getOcularDataDir: () => getOcularDataDir,
  getShiftleftConfigPath: () => getShiftleftConfigPath2,
  getWindsurfHookHandlerPath: () => getWindsurfHookHandlerPath2,
  hasSupportedExtension: () => hasSupportedExtension,
  hasValidShiftleftConfig: () => hasValidShiftleftConfig2,
  isOcularInstalled: () => isOcularInstalled2,
  registerCursorHooks: () => registerCursorHooks2,
  registerWindsurfHooks: () => registerWindsurfHooks2,
  runHookScanPipeline: () => runHookScanPipeline2,
  runScan: () => runScan2,
  runScanAndGetDiagnostics: () => runScanAndGetDiagnostics
});
...
var START_MARKER = "===FINDINGS_JSON_START===";
var END_MARKER = "===FINDINGS_JSON_END===";
var DEFAULT_TIMEOUT_MS = 3e4;
var DEFAULT_MAX_BUFFER = 10 * 1024 * 1024;
async function runScan(filePath, options = {}) {
  const dataDir = options.dataDir;
  if (!dataDir) {
    throw new Error("runScan requires options.dataDir");
  }
  const timeout = options.timeout != null ? options.timeout : DEFAULT_TIMEOUT_MS;
  const maxFileSizeKB = options.maxFileSizeKB != null ? options.maxFileSizeKB : 1024;
  if (maxFileSizeKB > 0) {
    const stat = import_fs3.default.statSync(filePath);
    const fileSizeKB = stat.size / 1024;
    if (fileSizeKB > maxFileSizeKB) {
      throw new Error(`File too large (${Math.floor(fileSizeKB)}KB), max ${maxFileSizeKB}KB`);
    }
  }
  if (!isOcularInstalled(dataDir)) {
    throw new Error("Dependencies not installed; call ensureOcularInstalled first");
  }
  const ocularScript = getOcularScriptPath(dataDir);
  const analyzeScript = getAnalyzeScriptPath(dataDir);
  if (!import_fs3.default.existsSync(analyzeScript)) {
    throw new Error("analyze.sc not found in data directory");
  }
  const { stdout, stderr } = await execFilePromise2(
    "sh",
    [
      ocularScript,
      "--script",
  ...
  const startIdx = output2.indexOf(START_MARKER);
  const endIdx = output2.indexOf(END_MARKER);
  if (startIdx === -1 || endIdx === -1) {
    return [];
  }
  const jsonStr = output2.substring(startIdx + START_MARKER.length, endIdx).trim();
  if (!jsonStr) return [];
...
function getDistDir() {
  return import_path2.default.join(__dirname, "..", "..", "dist");
}
function getCursorHookHandlerPath() {
  return import_path2.default.join(getDistDir(), "cursor-hook-handler.js");
}
function getWindsurfHookHandlerPath() {
  return import_path2.default.join(getDistDir(), "windsurf-hook-handler.js");
}
function ensureHookAuthToken(tokenPath) {
  if (import_fs4.default.existsSync(tokenPath)) {
    return import_fs4.default.readFileSync(tokenPath, "utf8").trim();
  }
  const token = import_crypto2.default.randomBytes(32).toString("hex");
  const dir = import_path2.default.dirname(tokenPath);
  if (!import_fs4.default.existsSync(dir)) {
    import_fs4.default.mkdirSync(dir, { recursive: true, mode: 493 });
  }
  const oldUmask = process.umask(63);
  try {
    import_fs4.default.writeFileSync(tokenPath, token, { mode: 384 });
  } finally {
    process.umask(oldUmask);
  }
  return token;
}
function createHookSocketServer(options) {
  const { socketPath, onFindings, onLog } = options;
  const authToken = options.authToken != null ? options.authToken : ensureHookAuthToken(options.tokenPath);
  if (import_fs4.default.existsSync(socketPath)) {
    try {
      import_fs4.default.unlinkSync(socketPath);
    } catch (err) {
      if (onLog) onLog(`Failed to remove old socket: ${err.message}`);
    }
  }
  const server = import_net.default.createServer((connection) => {
    let buffer = "";
    connection.on("data", (chunk2) => {
      buffer += chunk2.toString();
      if (buffer.length > 50 * 1024 * 1024) {
        if (onLog) onLog("SECURITY: Connection exceeded size limit");
        connection.destroy();
      }
    });
    connection.on("end", () => {
      try {
        ...
        if (!data.authToken || data.authToken !== authToken) {
          if (onLog) onLog("SECURITY: Rejected unauthenticated connection");
          return;
        }
        if (!data.fileName || typeof data.fileName !== "string") return;
        if (!Array.isArray(data.findings) || data.findings.length > 1e3) return;
        onFindings(data.fileName, data.findings);
      } catch (err) {
        if (onLog) onLog(`Socket data error: ${err.message}`);
      }
    });
    connection.on("error", (err) => {
      if (onLog) onLog(`Connection error: ${err.message}`);
    });
  });
  const oldUmask = process.umask(63);
  server.listen(socketPath, () => {
    try {
      import_fs4.default.chmodSync(socketPath, 384);
    } finally {
      process.umask(oldUmask);
    }
  });
  return {
    ...
    dispose() {
      server.close();
      try {
        if (import_fs4.default.existsSync(socketPath)) import_fs4.default.unlinkSync(socketPath);
      } catch (_) {
      }
    }
  };
}
function getCursorStopHandlerPath() {
  return import_path2.default.join(getDistDir(), "cursor-stop-handler.js");
}
function writeHooksFile(options) {
  const { hooksFilePath, hooks, onLog, includeVersion = true } = options;
  try {
    let hooksConfig = includeVersion ? { version: 1, hooks: {} } : { hooks: {} };
    if (import_fs4.default.existsSync(hooksFilePath)) {
      hooksConfig = JSON.parse(import_fs4.default.readFileSync(hooksFilePath, "utf8"));
    }
    if (!hooksConfig.hooks) hooksConfig.hooks = {};
    for (const [event, { command, identifier }] of Object.entries(hooks)) {
      if (!Array.isArray(hooksConfig.hooks[event])) hooksConfig.hooks[event] = [];
      hooksConfig.hooks[event] = hooksConfig.hooks[event].filter(
        (h2) => !(h2.command && h2.command.includes(identifier))
      );
      hooksConfig.hooks[event].push({ command });
    }
    const dir = import_path2.default.dirname(hooksFilePath);
    if (!import_fs4.default.existsSync(dir)) {
      import_fs4.default.mkdirSync(dir, { recursive: true, mode: 493 });
    }
    const oldUmask = process.umask(63);
    try {
      import_fs4.default.writeFileSync(hooksFilePath, JSON.stringify(hooksConfig, null, 2), { mode: 384 });
    } finally {
      process.umask(oldUmask);
    }
    if (onLog) onLog(`Hooks registered in ${hooksFilePath}: ${Object.keys(hooks).join(", ")}`);
    return true;
  } catch (err) {
    if (onLog) onLog(`Failed to register hooks: ${err.message}`);
    return false;
  }
}
function registerCursorHooks(options) {
  return writeHooksFile({ ...options, includeVersion: true });
}
function registerWindsurfHooks(options) {
  return writeHooksFile({ ...options, includeVersion: false });
}

// ../packages/curness-core/lib/hookPipeline.js
var import_fs5 = __toESM(require("fs"), 1);
async function runHookScanPipeline(core, payload, options) {
  const { getPaths, ensureOcularIfMissing, onFindings, getOutput, onProgress } = options;
  const paths = (getPaths(payload) || []).filter(
    (p) => typeof p === "string" && p && core.hasSupportedExtension(p) && import_fs5.default.existsSync(p)
  );
  if (paths.length === 0) {
    return getOutput([]);
  }
  const dataDir = core.getOcularDataDir();
  if (!core.hasValidShiftleftConfig(dataDir)) {
    return getOutput([
      {
        filePath: paths[0],
        error: "Missing or invalid ~/.shiftleft/config.json. Need orgId (UUID) and accessToken (JWT)."
      }
    ]);
  }
  if (!core.isOcularInstalled(dataDir)) {
    if (ensureOcularIfMissing) {
      try {
        await core.ensureOcularInstalled({
          dataDir,
          onProgress: onProgress || (() => {
          })
        });
      } catch (err) {
        return getOutput([{ filePath: paths[0], error: err.message }]);
      }
    } else {
      return getOutput([
        { filePath: paths[0], error: "Dependencies not installed. Start a new session to download them." }
      ]);
    }
  }
  const results = [];
  for (const filePath of paths) {
    try {
      const findings = await core.runScan(filePath, {
        dataDir,
        timeout: 3e4,
        maxFileSizeKB: 1024
      });
      results.push({ filePath, findings });
      if (onFindings) onFindings(filePath, findings);
    } catch (err) {
...
// ../packages/curness-core/lib/pendingFindings.js
var import_fs6 = __toESM(require("fs"), 1);
var import_path3 = __toESM(require("path"), 1);
function appendPendingFindings(pendingPath, filePath, findings) {
  if (!findings || findings.length === 0) return;
  const dir = import_path3.default.dirname(pendingPath);
  try {
    import_fs6.default.mkdirSync(dir, { recursive: true, mode: 448 });
  } catch {
    return;
  }
  ...
  try {
    for (let i2 = 0; i2 < 50; i2++) {
      try {
        lockFd = import_fs6.default.openSync(
          lockPath,
          import_fs6.default.constants.O_CREAT | import_fs6.default.constants.O_EXCL | import_fs6.default.constants.O_WRONLY
        );
        break;
      } catch {
        const wait = (ms) => {
    ...
    let pending = [];
    try {
      pending = JSON.parse(import_fs6.default.readFileSync(pendingPath, "utf8"));
    } catch {
    }
    pending.push({ filePath, findings });
    import_fs6.default.writeFileSync(pendingPath, JSON.stringify(pending), { mode: 384 });
  } finally {
    if (lockFd != null) import_fs6.default.closeSync(lockFd);
    try {
...
function findingKey(finding) {
  var _a, _b, _c, _d, _e;
  const pairs = finding.keyValuePairs ?? [];
  const title = ((_a = pairs.find((kv) => kv.key === "TitleTemplate")) == null ? void 0 : _a.value) ?? "";
  const sinkLine = (_e = (_d = (_c = (_b = finding.evidence) == null ? void 0 : _b[0]) == null ? void 0 : _c.sink) == null ? void 0 : _d.callsite) == null ? void 0 : _e.lineNumber;
  return `${title}|${sinkLine ?? ""}`;
}
function buildFollowupMessage(pendingFindings, options = {}) {
  const extensionName = options.extensionName || "Curness";
  const byFile = /* @__PURE__ */ new Map();
  for (const { filePath, findings } of pendingFindings) {
    if (!findings || findings.length === 0) continue;
    if (!byFile.has(filePath)) byFile.set(filePath, { list: [], seen: /* @__PURE__ */ new Set() });
    const { list, seen } = byFile.get(filePath);
  ...
  const lines = [];
  let totalCount = 0;
  for (const [filePath, { list: findings }] of byFile) {
    if (findings.length === 0) continue;
    lines.push(`In ${import_path3.default.basename(filePath)}:`);
    findings.forEach((finding, i2) => {
      var _a, _b, _c, _d, _e, _f, _g;
      const pairs = finding.keyValuePairs ?? [];
      const title = ((_a = pairs.find((kv) => kv.key === "TitleTemplate")) == null ? void 0 : _a.value) ?? "Security issue detected";
      const score = parseFloat(((_b = pairs.find((kv) => kv.key === "Score")) == null ? void 0 : _b.value) ?? "0");
      const description = ((_c = pairs.find((kv) => kv.key === "DescriptionTemplate")) == null ? void 0 : _c.value) ?? "";
      const severity = score >= 7 ? "High" : score >= 4 ? "Medium" : "Low";
      const sinkLine = (_g = (_f = (_e = (_d = finding.evidence) == null ? void 0 : _d[0]) == null ? void 0 : _e.sink) == null ? void 0 : _f.callsite) == null ? void 0 : _g.lineNumber;
      const lineStr = sinkLine != null ? ` at line ${sinkLine}` : "";
      lines.push(`  ${i2 + 1}. ${title} (${severity})${lineStr}`);
  ...
  const header = `${extensionName} discovered ${totalCount} security issue${totalCount !== 1 ? "s" : ""}:`;
  return [header, "", ...lines, "Fix them."].join("\n");
}
function getClaudePendingPath(payload) {
  const base = process.env.CLAUDE_PROJECT_DIR || payload && payload.cwd || process.cwd();
  return import_path3.default.join(base, ".claude", "curness", "pending-findings.json");
}
...
function getShiftleftConfigPath(dataDir) {
  const dir = dataDir || import_path4.default.join(import_os2.default.homedir(), ".shiftleft");
  return import_path4.default.join(dir, "config.json");
}
function hasValidShiftleftConfig(dataDir) {
  const configPath = getShiftleftConfigPath(dataDir);
  if (!import_fs7.default.existsSync(configPath)) return false;
  let config;
  try {
    config = JSON.parse(import_fs7.default.readFileSync(configPath, "utf8"));
  } catch {
    return false;
  }
...
// ../packages/curness-core/index.js
var __dirname3 = import_path6.default.dirname((0, import_url3.fileURLToPath)(__import_meta_url__));
var _require = (0, import_module.createRequire)(__import_meta_url__);
function getDiagnostics() {
  return _require("./lib/diagnostics.js");
}
var CURNESS_EXTENSION_ID = "curness.curness";
...
function hasSupportedExtension(filePath) {
  return typeof filePath === "string" && SUPPORTED_EXTENSIONS.some((ext) => filePath.endsWith(ext));
}
function extractPathsFromEditWriteToolInput(toolInput, cwd) {
  if (!toolInput || typeof toolInput !== "object") return [];
  const raw = toolInput.path ?? toolInput.file_path ?? toolInput.path_to_file ?? toolInput.filePath;
  if (!raw) return [];
  const cwdStr = cwd || process.cwd();
  const resolve = (p) => import_path6.default.isAbsolute(p) ? p : import_path6.default.resolve(cwdStr, p);
  if (Array.isArray(raw)) {
    return raw.map(resolve).filter(Boolean);
  }
  return [resolve(String(raw))];
}
function getOcularDataDir() {
  return import_path6.default.join(import_os3.default.homedir(), ".shiftleft");
}
function ensureAnalyzeScript2(dataDir) {
  const analyzeScriptSource2 = import_path6.default.join(__dirname3, "analyze.sc");
  return ensureAnalyzeScript(dataDir, analyzeScriptSource2);
}
async function ensureOcularInstalled2(options = {}) {
  const dataDir = options.dataDir || getOcularDataDir();
  const analyzeScriptSource2 = options.analyzeScriptSource ?? import_path6.default.join(__dirname3, "analyze.sc");
...
async function runScan2(filePath, options = {}) {
  const dataDir = options.dataDir || getOcularDataDir();
...
async function runScanAndGetDiagnostics(filePath, options = {}) {
  const dataDir = options.dataDir || getOcularDataDir();
  const findings = await runScan2(filePath, {
    dataDir,
    timeout: options.timeout,
    maxFileSizeKB: options.maxFileSizeKB
  });
  return getDiagnostics().findingsToDiagnostics(filePath, findings, {
    source: options.source,
    diagnosticSource: options.diagnosticSource,
    showSource: options.showSource
  });
}
function findingsToDiagnostics(analyzedFile, findings, options = {}) {
  return getDiagnostics().findingsToDiagnostics(analyzedFile, findings, options);
}
async function ensureOcularReady2(options = {}) {
  const ocularReady = await Promise.resolve().then(() => (init_ocularReady(), ocularReady_exports));
  return ocularReady.ensureOcularReady(options);
}
function isOcularInstalled2(dataDir) {
  return isOcularInstalled(dataDir || getOcularDataDir());
}
function formatFindingsSummary(filePath, findings, options = {}) {
  const extensionName = options.extensionName || "Curness";
  const base = import_path6.default.basename(filePath);
  if (!findings || findings.length === 0) {
    return `${extensionName}: no issues in ${base}`;
  }
  return `${extensionName}: ${findings.length} issue(s) in ${base}`;
}
var OCULAR_VERSION2 = OCULAR_VERSION;
...
var getShiftleftConfigPath2 = getShiftleftConfigPath;
var appendPendingFindings2 = appendPendingFindings;
var buildFollowupMessage2 = buildFollowupMessage;
var getClaudePendingPath2 = getClaudePendingPath;
var runHookScanPipeline2 = runHookScanPipeline;
var getCursorHookHandlerPath2 = getCursorHookHandlerPath;
var getCursorStopHandlerPath2 = getCursorStopHandlerPath;
var getWindsurfHookHandlerPath2 = getWindsurfHookHandlerPath;
var ensureHookAuthToken2 = ensureHookAuthToken;
var createHookSocketServer2 = createHookSocketServer;
var registerCursorHooks2 = registerCursorHooks;
var registerWindsurfHooks2 = registerWindsurfHooks;
...
function acceptPayload(payload) {
  if (payload.hook_event_name !== "PostToolUse") return false;
  const toolName = payload.tool_name;
  return toolName && /^(Edit|Write)$/i.test(toolName);
}
var input = "";
...
process.stdin.on("end", () => {
  ...
  (async () => {
    const messages = [];
    const pendingPath = getClaudePendingPath2(payload);
    const result = await runHookScanPipeline2(curness_core_exports, payload, {
      getPaths: (p) => extractPathsFromEditWriteToolInput(p.tool_input, p.cwd),
      ensureOcularIfMissing: false,
      onProgress: (msg) => process.stderr.write(msg + "\n"),
      onFindings(filePath, findings) {
        messages.push(
          formatFindingsSummary(filePath, findings, {
            extensionName: "Harness SAST and SCA"
          })
        );
        appendPendingFindings2(pendingPath, filePath, findings);
      },
      getOutput(results) {
        for (const r of results) {
          if (r.error) {
            messages.push(
              `Harness SAST and SCA: ${import_path7.default.basename(r.filePath)} \u2013 ${r.error}`
            );
          }
        }
        if (messages.length > 0) return { systemMessage: messages.join(" ") };
        return {};
      }
    });
    output(result);
    ...
    output({
      systemMessage: `Harness SAST and SCA: ${err.message}`
    });
    process.exit(0);
  });
});

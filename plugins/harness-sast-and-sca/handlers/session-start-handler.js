...
process.stdin.on("end", () => {
  ...
  (async () => {
    ...
    if (eventName !== "SessionStart") {
      process.stderr.write(`Harness SAST and SCA SessionStart: skipped (event=${eventName ?? "none"})
`);
      output({
        hookSpecificOutput: {
          hookEventName: "SessionStart",
          curnessStatus: `skipped_event_${eventName ?? "none"}`,
          additionalContext: "[Harness SAST and SCA] SessionStart hook skipped (wrong event)."
        }
      });
      return;
    }
    ...
    if (!hasValidShiftleftConfig2(dataDir)) {
      process.stderr.write(
        "Harness SAST and SCA SessionStart: skipped (no valid ~/.shiftleft/config.json)\n"
      );
      output({
        hookSpecificOutput: {
          hookEventName: "SessionStart",
          curnessStatus: "skipped_no_config",
          additionalContext: "[Harness SAST and SCA] Skipped: add ~/.shiftleft/config.json (orgId + accessToken) to enable dependency download."
        }
      });
      return;
    }
    ...
    try {
      if (!isOcularInstalled2(dataDir)) {
        process.stderr.write(
          "Harness SAST and SCA SessionStart: Dependencies not installed, downloading...\n"
        );
        ...
        if (!ok) {
          process.stderr.write(
            "Harness SAST and SCA: Dependencies download failed. Run with --debug to see details.\n"
          );
          output({
            hookSpecificOutput: {
              hookEventName: "SessionStart",
              curnessStatus: "install_failed",
              additionalContext: "[Harness SAST and SCA] Dependencies download failed. See debug log or run the session-start handler manually for details."
            }
          });
          return;
        }
        didInstall = true;
        process.stderr.write(
          "Harness SAST and SCA SessionStart: Dependencies installed.\n"
        );
      } else {
        const { updateAvailable } = await checkOcularUpdate2(dataDir);
        if (updateAvailable) {
          await ensureOcularInstalled2({
            dataDir,
            analyzeScriptSource,
            forceUpdate: true,
            onProgress: (msg) => process.stderr.write(msg + "\n")
      ...
      process.stderr.write(
        `Harness SAST and SCA SessionStart: ${err.message}
`
      );
      output({
        hookSpecificOutput: {
          hookEventName: "SessionStart",
          curnessStatus: `error_${err.message}`,
          additionalContext: `[Harness SAST and SCA] Error: ${err.message}`
        }
      });
      return;
    }
    output({
      hookSpecificOutput: {
        hookEventName: "SessionStart",
        curnessStatus: "ok",
        additionalContext: didInstall ? "[Harness SAST and SCA] Dependencies installed (~400MB). Security analysis will run after Edit/Write." : "[Harness SAST and SCA] Dependencies are ready. Security analysis runs automatically after Edit/Write."
      }
    });
  })().then(() => process.exit(0)).catch((err) => {
    process.stderr.write(
      `Harness SAST and SCA SessionStart: ${(err == null ? void 0 : err.message) || err}
`
    );
    output({});
    process.exit(0);

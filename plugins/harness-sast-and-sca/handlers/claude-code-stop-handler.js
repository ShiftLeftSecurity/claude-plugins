...
var import_child_process, import_semver, import_util, execFilePromise, OCULAR_VERSION, OCULAR_URL;
...
// src/claude-code-stop-handler.js
...
var __dirname = import_path.default.dirname((0, import_url.fileURLToPath)(__import_meta_url__));
...
function buildFollowupMessage(pendingFindings, options = {}) {
  ...
  for (const [filePath, { list: findings }] of byFile) {
    ...
    lines.push(`In ${import_path2.default.basename(filePath)}:`);
    findings.forEach((finding, i) => {
      ...
      lines.push(`  ${i + 1}. ${title} (${severity})${lineStr}`);
...
function getClaudePendingPath(payload) {
  ...
  return import_path2.default.join(base, ".claude", "curness", "pending-findings.json");
}
...
function getShiftleftConfigPath(dataDir) {
  const dir = dataDir || import_path3.default.join(import_os.default.homedir(), ".shiftleft");
...
function hasValidShiftleftConfig(dataDir) {
  ...
  if (!import_fs.default.existsSync(configPath)) return false;
  let config;
  try {
...
function getOcularDataDir() {
  return import_path4.default.join(import_os2.default.homedir(), ".shiftleft");
}
...
process.stdin.on("end", () => {
  ...
  try {
    ...
    let pendingFindings;
    try {
      pendingFindings = JSON.parse(import_fs2.default.readFileSync(pendingPath, "utf8"));
    } catch (readErr) {
    ...
    try {
      import_fs2.default.unlinkSync(pendingPath);
    } catch (unlinkErr) {
      if ((unlinkErr == null ? void 0 : unlinkErr.code) !== "ENOENT") throw unlinkErr;
    }
    const message = buildFollowupMessage2(pendingFindings, {

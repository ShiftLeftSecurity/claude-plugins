...
- **Never** run `sl_*` via **Bash** — the name is not a shell command and there is no `handlers/mcp__...js` script.
- **Bash is only** for setup (`claude mcp add`, `node scripts/doctor.mjs`, `sleep` while polling AutoFix) — not for `sl_ensure_cli`, `sl_analyze`, or other `sl_*` MCP tools.

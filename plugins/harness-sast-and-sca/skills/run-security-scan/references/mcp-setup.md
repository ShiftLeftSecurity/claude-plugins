...
- If the host reports a tool timeout but `~/.shiftleft/sl` appears afterward, retry the **MCP** scan — the install may have finished after the client gave up.
- A host timeout on **`sl_analyze`** (or **Not connected**) is **not** a CLI/API fallback. Wait and retry **MCP** `sl_list_branch_scans`. Never Shell `sl`, never read `~/.shiftleft/config.json`, never call apiv4 with `accessToken`. Ask the user to reload the agent host if the server stays down.
...
After adding, **restart Claude Code** (exit and reopen the app), then run `/mcp` to verify connection. A new chat in the same running process usually does **not** load new MCP servers. Tools are prefixed: `mcp__harness-code-security-mcp__sl_whoami`.
...
After saving MCP config, **restart Cursor** (quit/reopen or **Developer: Reload Window**) before expecting `sl_*` tools.
...
The plugin includes **`mcp.json`** that starts bundled **`slmcp/slmcp.cjs`** (no `node_modules`). Cloud skills ship in **`skills/`** (`/run-security-scan`, `/triage-vuln`, `/autofix-vuln`).
...
Same bundled **`slmcp/slmcp.cjs`**. Optional: install cloud skills via **`npx skills add`** if not using the plugin tree.

## Devin

### Devin CLI

Devin CLI has no `--plugin-dir` flag. Add the built **`release/curness-devin-plugin/`** path to **`plugin_dirs`** in **`~/.config/devin/config.json`** (or **`/config`** in a `devin` session), run **`handlers/install-harness-sast-devin-hooks.js`** once, then start a **new** `devin` session. Not the same as **`devin plugins install`** (skills-only Devin plugins). See [claude-plugin/README.md](../../../claude-plugin/README.md#devin).

### Devin Desktop (Devin Local)

Install the Curness VSIX and select **Devin Local** in the agent picker (not **Cascade**, which uses Windsurf-style hooks). Optional **`plugin_dirs`** entry for **`release/curness-devin-plugin/`** adds bundled MCP and cloud skills via **`~/.config/devin/config.json`**.
...
- **`sl_*` tools are MCP tools** — call them with a native tool invocation after ToolSearch loads deferred tools.
- Use the **exact prefixed name** from ToolSearch (e.g. `mcp__harness-code-security-mcp__sl_analyze`).
- **Never** run `sl_*` via **Bash** — the name is not a shell command and there is no `handlers/mcp__...js` script. Do not run `~/.shiftleft/sl` either.
- **Never** read `~/.shiftleft/config.json` or call the Qwiet API from Shell. Credentials stay inside MCP.
- **Bash is only** for setup (`claude mcp add`, `node scripts/doctor.mjs`, `sleep` while polling AutoFix) — not for `sl_ensure_cli`, `sl_analyze`, `sl_list_findings`, or other `sl_*` MCP tools.
